...
# @st.cache_resource
# def get_connection():
#     return sqlite3.connect("C:/Users/Bob/OneDrive/StX_XC_Track/track_analytics_app/data/stx_track.db", check_same_thread=False)
@st.cache_resource
def get_connection():
    """Connects to the SQLite database using a relative path."""
...
def format_time(seconds):
    """
    Robustly formats seconds into MM:SS.ss, handling non-numeric input.
    """
    try:
...
def highlight_laps_only(df):
    """
    Original, simple highlighter for the All Teams tab. 
    Only highlights min/max laps and knows nothing about PRs.
    """
    df_styled = df.copy()
    lap_cols = [col for col in df_styled.columns if 'Lap' in col]
    if not lap_cols: return df_styled.style
    
    # Format lap columns before applying styles
...
def apply_all_styles(row):
    """
    Advanced highlighter for the St. X tab. Handles PRs and laps.
    """
    styles = [''] * len(row)
...
tab1, tab3, tab2 = st.tabs(["St. X Meet Report", "Meet Results (All Teams)", "Athlete Report"])
...
with tab1:
    ...
    if selected_race_id_tab1:
        # ... (Data fetching and PR logic is unchanged and correct)
        ...
        if not race_df_tab1.empty:
            ...
            if athletes_in_meet:
                ...
                pr_df = pd.read_sql_query(pr_query, conn, params=(athletes_in_meet + [selected_race_date]))
            else:
                pr_df = pd.DataFrame(columns=['competitor_name', 'event', 'pr_sec'])
            ...
            for event in final_sorted_events:
                ...
                # ... (Navigation section is unchanged and correct)
                st.write("---")
...
# ============================================================
# TAB 2: ATHLETE REPORT (Full Code Restored)
# ============================================================
with tab2:
    ...
    try:
        ...
        try:
            ...
            default_index = 0
        selected_athlete = st.selectbox("Select an Athlete", options=athlete_names, index=default_index, key='athlete_select')
    except Exception as e:
    ...
    if selected_athlete:
        ...
        with col1:
            ...
            pr_query = "SELECT event, MIN(total_time_sec) as mark FROM v_all_results WHERE competitor_name = ? GROUP BY event ORDER BY event"
            pr_df = pd.read_sql_query(pr_query, conn, params=(selected_athlete,))
            pr_df['Mark'] = pr_df['mark'].apply(format_time)
            st.dataframe(pr_df[['event', 'Mark']].rename(columns={'event':'Event'}), hide_index=True, use_container_width=True)
        with col2:
            st.subheader("Season Bests")
            year_query = "SELECT DISTINCT race_year FROM v_all_results WHERE competitor_name = ? ORDER BY race_year DESC"
            year_df = pd.read_sql_query(year_query, conn, params=(selected_athlete,))
            if not year_df.empty:
                selected_season = st.selectbox("Select Season", options=year_df['race_year'].tolist())
                sb_query = "SELECT event, MIN(total_time_sec) as mark FROM v_all_results WHERE competitor_name = ? AND race_year = ? GROUP BY event ORDER BY event"
                sb_df = pd.read_sql_query(sb_query, conn, params=(selected_athlete, selected_season))
                sb_df['Mark'] = sb_df['mark'].apply(format_time)
                st.dataframe(sb_df[['event', 'Mark']].rename(columns={'event':'Event'}), hide_index=True, use_container_width=True)
            else:
                st.write("No season data found.")
        st.subheader("All Results")
        all_results_base_query = "SELECT race_date, race_name, event, total_time_text, place_in_level FROM v_all_results WHERE competitor_name = ?"
        athlete_events_df = pd.read_sql_query("SELECT DISTINCT event FROM v_all_results WHERE competitor_name = ?", conn, params=(selected_athlete,))
        event_filter_options = ['All'] + sorted(athlete_events_df['event'].unique().tolist())
        selected_event_filter = st.radio("Filter by Event:", options=event_filter_options, horizontal=True, key='event_filter_athlete_tab')
        if selected_event_filter == 'All':
            final_query = all_results_base_query + " ORDER BY race_date DESC"
            params = (selected_athlete,)
        else:
            final_query = all_results_base_query + " AND event = ? ORDER BY race_date DESC"
            params = (selected_athlete, selected_event_filter)
        all_results_df = pd.read_sql_query(final_query, conn, params=params)
        
        # --- FIX: Display the correct dataframe without styling ---
        if not all_results_df.empty:
            all_results_df.rename(columns={'race_date': 'Date', 'race_name': 'Meet', 'event': 'Event', 'total_time_text': 'Mark', 'place_in_level': 'Place'}, inplace=True)
            st.dataframe(all_results_df, hide_index=True, use_container_width=True)
        else:
            st.warning(f"No results found for the '{selected_event_filter}' event.")

# ============================================================
# TAB 3: MEET RESULTS (ALL TEAMS) - (Full Code Restored)
# ============================================================
with tab3:
    ...
    if selected_race_id_tab3:
        ...
        if not race_df_tab3.empty:
            ...
            for event in events_to_display:
                ...
                # --- FIX: Use the simpler highlighter for this tab ---
